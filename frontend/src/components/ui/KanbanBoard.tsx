...
import { ExternalLink, ArrowRight, Trash2, Loader2, Info, Calendar, MapPin, Briefcase, RotateCw } from 'lucide-react';
...
const KanbanBoard = () => {
    ...
    return (
        <div className="space-y-8 min-h-[calc(100vh-12rem)] bg-retro-cream p-1">
            {/* Tab Toggles */}
            <div className="flex space-x-4 border-b-3 border-black pb-5">
                <button
                    onClick={() => setActiveTab('active')}
                    className={`px-5 py-2.5 rounded-xl border-3 border-black font-black text-[10px] uppercase tracking-wider transition-all duration-100
                        ${activeTab === 'active' 
                ...
                >
                    Active Board ({jobs.filter(j => j.status !== 'rejected').length})
                </button>
                <button
                ...
                >
                    Archived Jobs ({jobs.filter(j => j.status === 'rejected').length})
                </button>
            </div>
            ...
            {activeTab === 'active' ? (
                ...
                <div className="bg-white border-3 border-black rounded-xl p-8 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] min-h-[50vh]">
                    <h3 className="text-sm font-black uppercase tracking-widest text-black mb-6">Archived Applications</h3>
                    
                    {jobs.filter(j => j.status === 'rejected').length === 0 ? (
                        <div className="py-20 text-center border-3 border-dashed border-black/30 rounded-xl bg-retro-cream/20">
                            <Trash2 className="w-10 h-10 text-black/35 mx-auto mb-4 animate-bounce" />
                            <p className="text-black/60 font-black text-sm italic">No archived jobs found.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {jobs.filter(j => j.status === 'rejected').map((job) => (
                                <div key={job.id} className="bg-white border-2 border-black rounded-lg p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="min-w-0 pr-2">
                                                <h4 className="font-extrabold text-black text-[13px] leading-tight truncate">{job.title}</h4>
                                        ...
                                        <div className="flex items-center space-x-3 text-[10px] text-black/60 font-bold mb-4">
                                            <div className="flex items-center">
                                                <MapPin className="w-3.5 h-3.5 mr-1 text-black/40" />
                                                {job.location}
                                            </div>
                                    ...
                                    <div className="pt-3 border-t-2 border-black/10 flex justify-end">
                                        <button 
                                            onClick={() => updateStatus(job.id, 'interested')}
                                            className="bg-retro-green text-white text-[9px] font-black uppercase px-3 py-1.5 rounded border-2 border-black shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[0.5px] hover:translate-y-[0.5px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1.5px] active:translate-y-[1.5px] active:shadow-none transition-all flex items-center"
                                        >
